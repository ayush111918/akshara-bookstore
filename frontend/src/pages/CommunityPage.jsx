...
function CommunityPage() {
  ...
  const visibleReviews = useMemo(() => {
    ...
    const filtered = normalizedQuery
      ? reviews.filter((review) => [review.bookTitle, review.readerName, review.headline, review.content]
      ...
      <div className="container-xl">
        ...
        {!loading && !error && reviews.length > 0 && visibleReviews.length === 0 && (
          <div className="empty-reader-page community-no-results">
            <i className="bi bi-search" />
            <h2>No reviews match that search</h2>
            <p>Try a book title, reader name, or a different phrase.</p>
            <button type="button" className="btn btn-ink" onClick={() => setQuery('')}>Clear search</button>
          </div>
        )}
